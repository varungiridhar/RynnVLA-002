...
class FinetuneSolverBase_ContinuousHead_Image_Gen(ABC):
    ...
    def get_args_parser(cls):
        ...
        parser.add_argument("--loss_awm_weight", dest="loss_awm_weight", type=float, default=0.0)
    ...
    def _dataset_func_wo_processed(self):
        ...
        dataset_train = LiberoFinetuneConversation(
            self.args.data_config_train, resolution=self.args.resolution, with_state=self.args.with_state, with_wrist=self.args.with_wrist, with_action=self.args.with_action, with_world_model=self.args.with_world_model, with_joint_awm=True
        )
        dataset_val_ind = LiberoFinetuneConversation(
            self.args.data_config_val_ind, resolution=self.args.resolution, with_state=self.args.with_state, with_wrist=self.args.with_wrist, with_action=self.args.with_action, with_world_model=self.args.with_world_model, with_joint_awm=True
        )
        dataset_val_ood = LiberoFinetuneConversation(
            self.args.data_config_val_ood, resolution=self.args.resolution, with_state=self.args.with_state, with_wrist=self.args.with_wrist, with_action=self.args.with_action, with_world_model=self.args.with_world_model, with_joint_awm=True
        )
        return dataset_train, dataset_val_ind, dataset_val_ood
    ...
    def train_one_epoch_awm(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            if len(batch_data)==2:
                ...
                labels = []
                future_images = []
                conversations, images, actions, states, future_imgs = batch_data
                if self.args.with_state:
    ...
    def train_one_epoch_awm_w(
        ...
        for data_iter_step, batch_data in enumerate(
            ...
            with {
                ...
                c_loss, additional_loss_dict, logits, hidden_states, labels_c, predicted_actions, loss_ct, image_logits, loss_awm = self.model(input_ids=examples, labels=labels, output_hidden_states=True, training=True, loss_weights=loss_weights, att_mask=True, get_future_state=True, target_image_tokens=future_images)
            ...
            loss = c_loss + self.args.loss_ct_weights * loss_ct + self.args.loss_awm_weight * loss_awm
            for add_loss, weight in additional_loss_dict.values():
                loss = loss + add_loss * weight
            loss_value = loss.item()
            c_loss_value = c_loss.item()
